```{r user-input}
  rm(list = ls(all = T))
  path.dropbox              <- "/Users/simonbogutzky/Privat/Dropbox/"
...
# plot relation
  file.path     <- paste(path.dropbox, "data-flow/final-data/", path.activity, path.person, "flow-hrv-relation-cp.pdf", sep = "")
  pdf(file.path, width = 11, height = 8, paper = "a4")
      
  par(mfrow = c(2, 1))
  PlotRelation(data.matrix$lfpowfft, data.matrix$absorption, main = "", xlab =  expression("Power LF-Band " (ms^2)), ylab = "Absorption", ylim = c(2, 7), pch = 20)

  text(data.matrix$lfpowfft, data.matrix$absorption, as.POSIXct(data.matrix$timestamp / 1000, origin = "1970-01-01"), cex = 0.5, pos = 1, col = terrain.colors(6)[2])
  
  title(main = paste("Relation - Activity: ", activity, ", Sex: ", sex, ", Age: ", age, sep = ""))
  
  PlotRelation(data.matrix$hfpowfft, data.matrix$absorption, main = "", xlab = expression("Power HF-Band " (ms^2)), ylab = "Absorption", ylim = c(2, 7), pch = 20)

  text(data.matrix$hfpowfft, data.matrix$absorption, as.POSIXct(data.matrix$timestamp / 1000, origin = "1970-01-01"), cex = 0.5, pos = 1, col = terrain.colors(6)[2])
