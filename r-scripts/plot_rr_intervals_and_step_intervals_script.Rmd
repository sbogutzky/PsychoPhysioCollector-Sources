    ...
    if(file.exists(file.path)) {
      ...
      for (internal.subset in 1:subsets.internal) {
       ...
       # par(mfrow = c(2, 1), mar = c(5.1, 5.1, 4.1, 2.1))
        ...
        file.path     <- paste(path.dropbox, "data-flow/final-data/", path.activity, path.person, substr(path.date, 1, 19), "-", "subset-", subset, "-", internal.subset, "-rr-intervals.csv", sep ="")
        write.csv(rr.intervals.subset, file.path, row.names = FALSE)
       ...
       # title(main = paste("Tachogram - Activity: ", activity, ", Sex: ", sex, ", Age: ", age, ", Subset ", subset, "/",  subsets.total, " - ", internal.subset, sep = ""))
        ...
        file.path     <- paste(path.dropbox, "data-flow/final-data/", path.activity, path.person, substr(path.date, 1, 19), "-", "subset-", subset, "-", internal.subset, "-step-intervals.csv", sep ="")
        write.csv(step.intervals.subset, file.path, row.names = FALSE)
