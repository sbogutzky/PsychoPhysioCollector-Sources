```{r user-input}
  rm(list = ls(all = T))
  path.name           <- "/Users/simonbogutzky/Downloads/"
  directory.name      <- "2014-05-27_12-28-37"
  input_filename      <- "sensor_BD38"
  output_filename     <- "sensor_BD38"
  subset_length_in_m  <- 15
```

```{r correction}
sensor.data <- read.csv(paste(path.name, directory.name, "/", input_filename, ".csv", sep =""))
...
write.csv(sensor.data, paste(path.name, directory.name, "/", output_filename, ".csv", sep =""), row.names = F)
```
...
filename.pattern <- "sensor_BD38_subset_1_0_5_hrv.txt"
file.list <- list.files(path = paste(path.name, directory.name, sep =""), pattern = filename.pattern, recursive = T)


sensor.bd38.subset.hrv <- read.csv(paste(path.name, directory.name, "/", file.list[1], sep =""), header = F, na.strings = "", fill = T, skip = 97, col.names = c("NA1", "Time", "RRInterval", "FFTFrequency", "FFTPSD", "ARFrequency", "ARPSD", "NA2", "NA3", "NA4", "NA5"))
sensor.bd38.subset.hrv <- sensor.bd38.subset.hrv[!is.na(sensor.bd38.subset.hrv$RRInterval),]

```

```{r extract-hrv-parameters}

hrv.data <- read.csv(paste(path.name, directory.name, "/", "sensor_BD38_subset_1_0_5_hrv_spss.txt", sep =""))

```


```{r extract-step-intervals}
motion.sensor.data.subset   <- read.csv(paste(path.name, directory.name, "/sensor_BC98_subset_1.csv", sep =""))

motion.sensor.data.subset.1 <- motion.sensor.data.subset[motion.sensor.data.subset$Timestamp < 5 * 60 * 1000, ]
date.start                  <- motion.sensor.data.subset.1$System.Timestamp[1] / 1000
date.end                    <- motion.sensor.data.subset.1$System.Timestamp[nrow(motion.sensor.data.subset.1)] / 1000

m   <- 1
n   <- nrow(motion.sensor.data.subset.1)
...
title(main = paste("Activity: Walking, Sex: Female, Start: ", as.POSIXct(date.start, tz = "Europe/Berlin", origin = "1970-01-01"), ", End: ", as.POSIXct(date.end, tz = "Europe/Berlin", origin = "1970-01-01"), ", Subset 1/4", sep = ""), sub = "LF: / HF: / LF/HF: / ")

plot(t[indexes] / 1000, c(NA, diff(t[indexes])) / 2000, type = "l", xlab = "Time (s)", ylab = "Step-to-Step Interval (s)", xaxs = "i", yaxs = "i")
