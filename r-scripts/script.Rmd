```{r user-input}
  ...
  person.path               <- "buse-patrick/"
  date.path                 <- "YYYY-mm-dd/" # YYYY-mm-dd/ for correcting ### YYYY-mm-dd_HH-MM-SS/ for subsetting 
  input.filename            <- "YYYY-mm-dd_HH-MM-SS-leg-data" # YYYY-mm-dd_HH-MM-SS-ecg-data or YYYY-mm-dd_HH-MM-SS-leg-data for correcting ### sensor_BD38 or sensor_BC98 for subsetting
  output.filename           <- "sensor_BC98" # sensor_BC98 for sensor_BD38 correcting ### leg-data or ecg-data for subsetting
...
```{r control}
file.path   <- paste(dropbox.path, "data-flow/processed-data/", activity.path, person.path, date.path, input.filename, ".csv", sep ="")
sensor.data <- read.csv(file.path)
n.col <- ncol(sensor.data)
summary(sensor.data)
...
file.source.path <- paste(dropbox.path, "data-flow/processed-data/", activity.path, person.path, date.path, "scale.csv", sep ="")
...
for(i in 1:nrow(scale)) {
  if(i == 1) {
    ...
    m <- nrow(sensor.data[sensor.data[, n.col] < scale[i - 1, 2],]) - 1
  }
  n <- m + l - 1
