```{r user-input}
  ...
  amplitude.range       <- c(0, 500)
  frequency.range       <- c(.033, .5)
...
```{r plot-rr-intervals-and-psd}
  ...
  # extract rr intervals and psd values 
    ...
    # calculate hrv parameters
      spss.hrv.data$meanrr
      CalculateMeanRR(rr.intervals, 4)
    
      spss.hrv.data$sdnn
      CalculateSDNN(rr.intervals, 4)
    
      spss.hrv.data$rmssd
      CalculateRMSSD(rr.intervals, 4)
    source("./functions.R")
      p <- CalculateHRVFrequencyDomainParameters(t, rr.intervals, band.vlf, band.lf, band.hf, 4, 256, .5)
...
#       }
      ...
      plot(p$Freq, p$Spec, type = "l", xlab = "Frequency (Hz)", ylab = "PSD (s^2/Hz)", xaxs = "i", yaxs = "i", xaxt = "n", ylim = c(0, 0.00025), xlim = frequency.range)
