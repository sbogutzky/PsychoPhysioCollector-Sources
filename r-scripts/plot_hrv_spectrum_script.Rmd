...
```{r hrv-parameter-history-against-flow}
  ...
  PlotFlow <- function() {
    par(new = T)
    plot(seq(from = 15, to = 60, by = 15), data.hrv.flow$flow[!is.na(data.hrv.flow$flow)], xlim = c(0, 60), ylim = c(2.5, 4.5), axes = F, xlab = NA, ylab = NA, type = "o", pch = 21, bg = terrain.colors(6)[2], cex = 2)
