    ...
    if(nrow(gps.data) > 0) {
        ...
        for (internal.subset in 1:subsets.internal) {
          ...
          if(nrow(gps.data.subset) > 0) {
            ...
            duplicatedLat <- duplicated(gps.data.subset[,2])
            duplicatedLon <- duplicated(gps.data.subset[,3])
            duplicatedAlt <- duplicated(gps.data.subset[,4])
            
            #korrekt: genau gleiche positionsdaten
            #duplicated <- duplicatedLat & duplicatedLon & duplicatedAlt
            
            duplicated <- duplicatedLat & duplicatedLon
            
            gps.data.subset <- gps.data.subset[!duplicated,]
                ...
                for (i in 2:nrow(gps.data.subset)) {
                 ...
                 # i <- 100
                  ...
                  lat1 <- gps.data.subset[i-1,2] * pi/180
                  lon1 <- gps.data.subset[i-1,3] * pi/180
                  lat2 <- gps.data.subset[i,2] * pi/180
                  lon2 <- gps.data.subset[i,3] * pi/180
                  d <- distance(lat1, lon1, lat2, lon2)
                  if(is.nan(d)) {
                    print(paste("Not a Number in Subset: ", subset, " Internal Subset: ", internal.subset, " Row: ", i))
                  }
                  distances <- c(distances, d)
                }
              
              # sum up
              absolved.kilometers <- sum(distances)
              absolved.meters <- absolved.kilometers*1000
...
#               duration <- end.time - start.time
            ...
            print("gps.data.subset", internal.subset  ," contains no data")
          } 
      }
      ...
      print(paste(file.path, " contains no data"))
    }
